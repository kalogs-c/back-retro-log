...
func (h *AuthHandler) LoginPage(w http.ResponseWriter, r *http.Request) {
	...
	ui.Layout("Login", false, ui.LoginPage("", count > 0)).Render(r.Context(), w)
}
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			ui.Layout("Login", false, ui.LoginPage("Invalid username or password", usersExist)).Render(r.Context(), w)
			return
		}
...
func (h *AuthHandler) RegisterPage(w http.ResponseWriter, r *http.Request) {
	...
	if token != "" {
		...
		if err != nil {
			errorMsg = "Invalid invitation token."
	...
	ui.Layout("Register", false, ui.RegisterPage(token, errorMsg)).Render(r.Context(), w)
}
...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" || password == "" {
		ui.Layout("Register", false, ui.RegisterPage(token, "Username and password are required.")).Render(r.Context(), w)
		return
	}

	existingUser, err := h.Queries.GetUserByUsername(r.Context(), username)
	if err == nil && existingUser.ID > 0 {
		ui.Layout("Register", false, ui.RegisterPage(token, "Username already taken.")).Render(r.Context(), w)
		return
	}
	...
	if count > 0 {
		...
		if err != nil || inv.UsedBy.Valid {
			ui.Layout("Register", false, ui.RegisterPage(token, "Invalid or expired invitation.")).Render(r.Context(), w)
			return
		}
		expiresAt, err := time.Parse(time.RFC3339, inv.ExpiresAt)
		if err != nil || time.Now().UTC().After(expiresAt) {
			ui.Layout("Register", false, ui.RegisterPage(token, "Invitation expired.")).Render(r.Context(), w)
			return
		}
		invID = inv.ID
	}
...
func (h *AuthHandler) InvitePage(w http.ResponseWriter, r *http.Request) {
	...
	ui.Layout("Invitations", true, ui.InvitePage(invitations, link)).Render(r.Context(), w)
}
